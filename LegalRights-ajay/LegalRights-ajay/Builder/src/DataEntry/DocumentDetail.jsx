...
export default function DocumentDetail() {
  ...
  const onFinish = (values) => {
    ...
    axios
      .post("/data-entry/create-detail-group", newData)
      .then((response) => {
  ...
  const handleSubmit = async (id, field) => {
    ...
    try {
      await axios.patch(`/data-entry/update-status/${id}`, {
        [field]: item[field],
      });
        ...
        <div style={{ display: "flex", gap: "20px" }}>
          ...
          <div>
            <Input
              value={record.eStamp}
              onChange={(e) =>
                handleInputChange(record._id, "eStamp", e.target.value)
              }
            />
            <Button onClick={() => handleSubmit(record._id, "eStamp")}>
              Submit
            </Button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "20px" }}>
          ...
          <div>
            <Input
              ...
              onChange={(e) =>
                handleInputChange(record._id, "eRegFee", e.target.value)
              }
            />
            <Button onClick={() => handleSubmit(record._id, "eRegFee")}>
              Submit
            </Button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "20px" }}>
          ...
          <div>
            <Input
              value={record.dCheck}
              onChange={(e) =>
                handleInputChange(record._id, "dCheck", e.target.value)
              }
            />
            <Button onClick={() => handleSubmit(record._id, "dCheck")}>
              Submit
            </Button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "20px" }}>
          ...
          <div>
            <Input
              ...
              onChange={(e) =>
                handleInputChange(record._id, "finalPrint", e.target.value)
              }
            />
            <Button onClick={() => handleSubmit(record._id, "finalPrint")}>
              Submit
            </Button>
          </div>
        </div>
